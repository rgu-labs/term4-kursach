...
Геометрическое распределение описывает число неудач до первого успеха. В работе используется его усечённая версия (конечный носитель) для генерации значений шага блуждания с убывающей вероятностью в задачах\nbsp{}4,\nbsp{}6,\nbsp{}7.
...
#+CAPTION: Генерация случайного шага $s$
#+BEGIN_SRC cpp
...
#+CAPTION: Метод run_single() --- генерация одной траектории
#+BEGIN_SRC cpp
...
Кластеры генерируются побуквенно методом обратного преобразования (inverse transform sampling) согласно заданному распределению вероятностей на алфавите.
...
*** Аналитическая вероятность для равномерного распределения
...
#+CAPTION: Законы распределения индекса направления $\xi_k$
...
#+CAPTION: Метод run_single() --- одно блуждание
#+BEGIN_SRC cpp
...
#+CAPTION: Функция байесовского анализа
#+BEGIN_SRC cpp
void analyze(
...
#+CAPTION: Метод run_single() --- одно бросание иглы
#+BEGIN_SRC cpp
...
*** Аналитический вывод вероятности пересечения
...
#+CAPTION: Характеристики случайного графа
...
*** Алгоритм Барейсса для числа остовных деревьев

#+CAPTION: Алгоритм Барейсса: определитель редуцированной матрицы Лапласа
#+BEGIN_SRC cpp
...
#+CAPTION: Испытание в одном потоке
#+BEGIN_SRC cpp
...
1. =OSMemory= --- резервирование/фиксация виртуальной памяти через =mmap=;
2. =VirtualHeap= --- резервирование непрерывного диапазона адресов с автоматической фиксацией страниц;
3. =ArenaAllocator= --- линейный аллокатор с сохранением и восстановлением состояния (стекинг);
4. =ArenaResource= --- адаптер =std::pmr::memory_resource= для использования с PMR-контейнерами.

#+CAPTION: Интерфейс аренного аллокатора
#+BEGIN_SRC cpp
